#!/usr/bin/env python3
"""
Optuna hyperparameter tuning for ExMRD_Retrieval on FakeTT and FakeSV datasets
...
class OptunaTuner:
    def __init__(self, dataset: str = "FakeTT", study_name: str = None, n_trials: int = 1000):
        ...
        if dataset == "FakeSV":
            ...
            self.text_encoder = "OFA-Sys/chinese-clip-vit-large-patch14"
        else:  # FakeTT
            self.fixed_params = {
                'include_piyao': False,
                'description': True,
    ...
    def suggest_hyperparameters(self, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        config = {
            'data': {
                'tokenizer_name': self.text_encoder,
                **self.fixed_params
            },
            'para': {
                'hid_dim': hid_dim,
    ...
    def save_best_results(self):
        ...
        filename = f'best_hyperparameters_{self.dataset.lower()}.json'
        with open(self.results_dir / filename, 'w') as f:
            json.dump(results, f, indent=2)
        
        self.logger.info(f"Best results saved: Accuracy = {self.best_accuracy:.4f} to {filename}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Optuna Hyperparameter Tuning for ExMRD_Retrieval')
    parser.add_argument('--dataset', type=str, default='FakeTT', choices=['FakeTT', 'FakeSV'],
    ...
    print(f"💾 Results saved to: optuna_results/best_hyperparameters_{DATASET.lower()}.json")
